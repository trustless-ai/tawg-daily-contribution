...
Local knowledge compilation may use the developer's local Codex identity. The Actions runner uses the operator-configured DeepSeek endpoint through the pinned Claude Code CLI; Claude sessions are not durable, so repository state is the only long-term context. Validate the workflow manually on the feature branch and push the workflow only after local acceptance.
